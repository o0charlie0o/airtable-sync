    ...
    return omitBy(
      {
        id:
          !isNil(destinationRecords) && !isNil(destinationIdRef) && isNil(recordIteratee)
            ? get(
                first(destinationRecords.filter((d) => d.fields[destinationIdRef] === r.id)),
                'id',
              )
            : !isNil(destinationRecords) && !isNil(recordIteratee)
            ? first(destinationRecords.filter((i) => recordIteratee(r, i)))?.id
            : null,
